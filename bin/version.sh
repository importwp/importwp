...
if [[ $(uname -s) == 'Darwin' ]]; then
	sed -i '' -E "s/^Stable tag: (.+)  $/Stable tag: $TAG  /g" readme.txt
	sed -i '' -E "s/^ \* Version: (.+) $/ \* Version: $TAG /g" jc-importer.php
	sed -i '' -E "s/^	define\('IWP_VERSION', '(.+)'\);$/	define('IWP_VERSION', '$TAG');/g" jc-importer.php
else
