...
<div class="wrap jci-page-wrapper <?php echo esc_attr( sprintf( 'jci-setting-%s-wrapper', $tab ) ); ?>">
    ...
    <h3 class="nav-tab-wrapper">
        ...
        <a href="<?php echo add_query_arg( 'tab', 'debug' ); ?>"
           class="nav-tab <?php if ( $tab == 'debug' ): ?>nav-tab-active<?php endif; ?>">Debug</a>
		<?php if ( ! class_exists( 'ImportWP_Pro' ) ): ?>
            <a href="<?php echo add_query_arg( 'tab', 'premium' ); ?>"
               class="nav-tab <?php if ( $tab == 'premium' ): ?>nav-tab-active<?php endif; ?>">Premium</a>
		<?php endif; ?>
    </h3>
